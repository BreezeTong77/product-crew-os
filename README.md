...
`run-routing-eval.rb` 会用 44 条标准 SOP gold case 和 20 条外部 gold case 计算真实路由指标，包括 Domain accuracy、Stage accuracy、Skill hit rate、Agent precision / recall 和 Domain exit accuracy。它调用统一的 `SemanticStageRouter`，不是把 expected stage 直接写进 runtime。
...
`sop-coverage-gap.md` 对 44 个 SOP 的路由、Skill 映射、Prompt 覆盖、Loop 复测和 runtime 证据做了最终汇总，可直接做发布前 sanity check。
