...
为了让用户读懂项目推进过程，Obsidian / Markdown 项目包用十类阶段目录做可视化归档：机会发现、用户研究、问题定义、需求分析、方案设计、PRD 与评审、交付规划、上线准备、上线监控、复盘迭代。它们不是“10 大流程已完整编排”的承诺；只会创建本次实际写入的目录。44 个 SOP 不作为用户可见一级目录，而是写入每个 artifact 的 `sop_id`、索引和事件日志；决策、风险、评审项、下一步和来源台账统一放入 `_项目账本/`，方便后续回溯和检索。
...
- 按实际写入的阶段目录导出 Obsidian-compatible Vault，不创建空阶段目录。
...
当前发布包已经通过本地写入验证：`run-runtime-smoke.rb` 会验证 SQLite、Project Workspace 和 Obsidian-compatible 导出；`run-sop-e2e-smoke.rb` 会遍历 44 个 SOP 写入 runtime trace。该 smoke 不产生通过类 Gate；真实 Skill、真实子 Agent 与用户确认由独立 E2E 门禁验证。

Runtime 还提供 `record-turn` adapter，用来把一次主控教练回合写成可查询的项目记录：

```text
Stage 判断 -> SOP / Skill 控制面校验 -> Artifact 写入 -> Context Packet -> 真实 Invocation Ledger -> 用户确认 -> Stage Gate Finalizer
```
...
`run-sop-e2e-smoke.rb` 会遍历 44 个 SOP prompt case，解析 Stage / Skill Router，并读取本地内置 skill 的 `SKILL.md` 验证映射与可读取性；它不等于执行了 44 个外部 Skill 工作流。真实 command 型 Skill 必须通过 `Skill Executor Registry` 返回 `execution_proof`，由 `run-skill-executor-smoke.rb` 验证；方法论型或 MCP 型 Skill 没有宿主回调/工具账本时必须显示 `unavailable`。
...
- [Skill Execution Contract](product-crew-os-skill/templates/skill-execution-contract.json)：让外部 Skill 保留专业工作流，同时禁止其改变 Stage、决定 Gate、写项目记忆或替代评审。
