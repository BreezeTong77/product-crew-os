...
| `test_case_runs` | 每次执行或跳过的流水记录（含 `expected_primary_skill`、`selected_skill`、`skill_status`、`degrade_reason`） |
...
- `PASS`：SOP 步骤成功、选到期望主技能（或 fallback）并完成 artifact/review 写入。
- `DEGRADED`：运行正常生成 artifact/review，但 `skill_status` 为 `template_used`（回退 `artifact-template`），用于记录可追溯降级，不视作可直接通过发布门禁。
- `FAIL`：SOP/写入链路异常，不满足预期行为。
- `SKIP_PASS`：已通过且指纹未变的历史 case，本次略过。
