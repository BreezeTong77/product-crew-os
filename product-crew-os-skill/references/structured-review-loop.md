...
主控教练可以组织评审、提炼冲突、建议下一步，但不能替用户决策，不能替用户采纳或拒绝评审意见，也不能把未解决的 block 擅自改成 pass。
...
用户决定采纳、拒绝、暂缓或补证据。主控教练只能识别意图、提示风险、执行用户确认后的动作；不能替用户决策，也不能把主控建议写成用户决定。
...
退出评审必须用户确认后关闭。即使所有角色都给出 pass 或 conditional pass，主控也只能建议关闭，不能自动替用户关闭 Review Session。
