...
| 5 | Evidence inventory | notes, data screenshots, prior docs | evidence inventory and confidence rating | Coach | Data for logs/data; Research for interviews; CS for field notes; Customer for direct demand/quote; Tech if system evidence is needed | QA by default | claims are separated from evidence gaps |
...
| 16 | Solution exploration | "方案怎么落地" | solution options brief | Coach | Design for UX path; Tech for feasibility; Data for data product; CS for adoption concern; Customer for external acceptance constraints | QA unless acceptance needed | at least 2 options and tradeoffs are visible |
| 17 | MVP scope | "先做 MVP" | MVP scope and not-do list | Coach, Biz | Tech for effort; Design for experience; Data if metric/data feature; Customer if external commitment constrains scope | Legal unless regulated | MVP can prove one core hypothesis |
| 18 | One-page proposal | "提案给业务方" | proposal one-pager | Coach, Biz | Data for metrics; CS for field language; Customer if external acceptance is involved | Tech only if feasibility is challenged; QA and Legal unless triggered | business owner can approve direction |
| 19 | Data feasibility precheck | data from other teams, model, recommendation, BI | data contract draft | Coach, Data | Tech if API/integration; Biz if data owner priority needed; Legal if personal/sensitive data | Design, QA | source, fields, freshness, ownership, SLA are known |
| 20 | Technical feasibility precheck | integration, automation, AI, complex logic | feasibility note and dependency list | Coach, Tech | Data if data-dependent; Design if surface complexity; QA if testability uncertain; Legal if security/compliance affects implementation | Biz unless scope negotiation | no unknown blocker prevents PRD writing |
| 21 | Compliance precheck | privacy, finance, messaging, contracts, risk controls | compliance risk note | Coach, Legal | Data if personal data; Biz if policy exception; Customer if external acceptance wording is involved | Design, QA | compliance constraints are known |
...
| 23 | Low-fi prototype | "做 demo / 原型" | low-fi prototype / clickable demo brief | Coach, Design | CS for adoption/usability; Customer for customer validation; Tech if component feasibility; Data if measurement points are embedded | QA, Legal | core screens and user decisions are testable |
...
| 25 | Instrumentation plan | "埋点怎么做" | event spec / tracking plan | Coach, Data | Tech for implementation; QA for validation if near delivery; Legal if privacy/personal data; Design if UI state affects trigger | Biz unless target changes | events, properties, timing, owner are specified |
...
| 28 | PM self-review | "我写完了先看看" | PM self-review checklist and fix list | Coach | Biz, Design, Tech, Data, or QA only if that domain issue is obvious | Full panel | PRD has no obvious contradiction before review |
| 29 | Internal product review | "组内先过" | review notes and revision plan | Coach | Biz if priority; Design if UX; Data if metrics; Tech if feasibility | Legal unless triggered; Full panel unless explicitly requested | blockers are classified: must-fix / should-fix / later |
| 30 | Design review | screen, flow, prototype, IA ready | design review report | Coach, Design | CS for usability; Tech for front-end constraints; Data if key behavior must be measured | Biz unless priority conflict; Legal unless risk | user path, states, and copy are clear |
| 31 | Data review | metrics, recommendation, ranking, attribution | data review report | Coach, Data | Tech for integration; Biz for target definitions; Legal for sensitive/personal data | Design unless dashboard UX | data contract is accepted or blockers named |
| 32 | Technical pre-review | "找研发看看" | tech pre-review notes | Coach, Tech | Data if source/API; Design if UI complexity; QA if testability; Legal if security/compliance implementation constraints | Biz unless scope negotiation | system boundary, dependencies, effort risks are clear |
| 33 | Formal requirements review | "正式评审" | approved PRD and decision log | Coach | Biz, Tech, Design, Data as domain owners; QA if acceptance is already in scope | Legal only if trigger; CS optional for adoption voice; Customer if external approval or contractual acceptance is needed | approve / conditional approve / reject is recorded |
...
| 36 | Development tracking | "研发中有变更" | change log and decision note | Coach | Tech always for implementation changes; Biz if scope/value changes; Design/Data if impacted; QA if acceptance or release risk changes | CS, Legal unless impact | change is accepted, deferred, or rejected |
| 37 | Integration / QA | "联调/测试" | bug list, QA report, release risk | Coach, QA | Tech; Data for event validation; Design for UX defects; Biz for go/no-go tradeoff | CS, Legal unless support, privacy, compliance, or external promise is impacted | critical issues resolved or signed off |
...
| 39 | Training / enablement | "怎么让一线用" | SOP, FAQ, training deck | Coach, Ops | CS, Biz; Design for help content; Tech if tool operation requires explanation; Data if adoption metrics need training | Customer unless customer-facing enablement is required | users know how to adopt the change |
| 40 | Grey release / pilot | "先试点" | pilot plan | Coach, Biz | Data, CS, Customer, Ops, QA; Tech for rollback/fallback; Legal if regulated or external promise exists | None by default | pilot scope, duration, metric, rollback set |
| 41 | Launch monitoring | "上线后看什么" | monitoring dashboard / daily notes | Coach, Data | Tech for incidents; CS/Ops for adoption signals; QA for escaped defects; Biz for go/no-go or success interpretation | Design unless UX issue | metrics and incidents are reviewed on schedule |
