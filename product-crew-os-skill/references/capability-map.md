...
| 项目资产沉淀 | 这个项目沉淀了什么 / 导出项目包 / 后面怎么检索 / 能不能给 Obsidian 看 | project asset pack, artifact workspace, export adapter | project home, artifact index, timeline, decision log, review items, export manifest |
...
- 每个阶段产生的重要 artifact、评审、决策和下一步要进入 Project Asset Pack，而不是散落在聊天中。

## 5. 项目资产包

Project Asset Pack 是 Product Crew OS 的项目级知识库与导出层。它把 44 个 SOP 中已经收束的项目资产沉淀为 Markdown/YAML/JSON：
...
Obsidian、Notion、飞书、Word、PDF 都是可选导出目标，不是运行时主容器。详细规则见 `project-asset-pack.md`。

## 6. 未来能力：语义阶段路由

Product Crew OS 的一个关键迭代方向是 Semantic Stage Router。

它要解决的问题不是“多一个 RAG 数据库”，而是让主控教练更稳定地理解用户自然语言背后的产品阶段：
...
-> 调用正确 SOP、skill、子 Agent 和 artifact
-> 记录用户纠错，形成 routing feedback
```

可分阶段实现：

- M0：规则 + LLM 分类 + JSON SOP 表，不依赖向量数据库。
- M1：本地 Markdown / JSON 轻量检索，查找相似 stage、SOP 和历史纠错。
- M2：Embedding / RAG，用于用户公司 SOP、历史项目、会议纪要、同事反馈和项目 artifact 检索。
- M3：长期反馈学习，让系统逐渐形成用户自己的产品办公室风格。

详细规则见 `semantic-stage-router.md`。
