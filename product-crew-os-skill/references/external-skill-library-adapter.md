...
Product Crew OS 可以评估和接入第三方 PM skill 包、团队内部模板或外部能力库，但不能把外部能力包直接变成产品本体。
...
6. 向用户说明推荐原因和风险。
7. 用户确认后，写入 User Preference Memory 或 Project Workspace overlay。
...
- 未经评估把外部几十个 skill 全部写入公共规则包。
- 未经用户确认自动安装。
- 把外部 skill 的具体示例、业务项目或作者表达复制进 Product Crew OS 公共规则。
