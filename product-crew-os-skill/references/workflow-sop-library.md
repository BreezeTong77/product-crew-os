...
如果某个阶段需要的 required / triggered roles 超过运行时的单批次评审上限，主控教练不能因为批次已满就丢掉必要角色。正确做法是按风险和 Stage Gate 影响拆成多批：第一批处理必需角色和可能卡门的角色，第二批处理体验、运营、客户采纳等补充视角。批次上限用于控制 token 和会议噪音，不用于改写 SOP 的 stakeholder 规则。
