...
Use `references/workflow-orchestration-policy.md` when deciding what belongs in the user frontstage, what must stay in the backend trace, how light/standard/deep mode works, or whether a Golden Case may be considered passed.
Use `references/workflow-state-machine.md` when the user asks whether workflow nodes, runtime status, state transitions, Obsidian visualization, or dashboard-style trace inspection can be seen.
Use `references/workflow-implementation-coverage-v0.md` before claiming Product Crew OS workflow completion, test coverage, Golden Case coverage, or full-state-machine readiness. This file is the source of truth for partial / missing workflow implementation status.
Treat macro-flow orchestration maps as experimental audit artifacts, not the default release gate. For normal Product Crew OS operation and regression, prefer the 44-SOP path: domain gate -> stage/SOP hit -> skill hit -> artifact/runtime write -> review ledger -> stage gate.
Use `references/semantic-stage-router.md` when the user's intent is ambiguous, route confidence is low, the user corrects a stage judgment, or routing/RAG/feedback-learning capability is being discussed. The current baseline uses an input scope gate plus the 44 SOP prompt-eval set as a local retrieval index before falling back to clarification; embedding/vector retrieval is optional and must not bypass memory isolation or stage gates.
...
Use `references/langgraph-runtime-architecture.md` when the user asks how the new control plane prevents a coach, Skill, review callback or checkpoint from skipping the required workflow sequence.
...
Use `runtime/pco_langgraph_runtime.py` as the preferred control plane when the user wants executable project memory, resumable stages, real external review callbacks, or an auditable Stage Gate. Its graph owns `Domain Gate -> Retrieval Evidence -> Stage/SOP -> Skill Guard -> Artifact -> Review Interrupt -> User Decision Interrupt -> Project Memory`; it pauses with a durable `thread_id` rather than letting the coach skip ahead in text. `runtime/pco_runtime.rb` remains the compatibility adapter for existing Ruby CLI, Coze Bridge and OCR/RAG paths during migration. A passing Gate still requires verified Skill evidence, complete required-role review evidence and a user confirmation; a graph checkpoint is not a pass by itself.
...
Use `runtime/pco_langgraph_runtime.py` as the first runnable control plane for new project-memory integrations. LangGraph SQLite checkpoints recover execution state only; the project SQLite database and Markdown/YAML/JSON files remain the audit and export source for project state, artifact versions, review records, memory deltas, context packets and invocation ledger entries. Keep `runtime/pco_runtime.rb` for existing Ruby-compatible integrations until their adapter tests move to LangGraph.
