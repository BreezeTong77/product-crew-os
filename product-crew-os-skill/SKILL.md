...
Use `runtime/pco_runtime.py` when the user wants executable project memory, resumable stages, real external review callbacks, OCR/RAG, controlled Skill execution, or an auditable Stage Gate. Its LangGraph owns `Domain Gate -> Project Context -> Retrieval Evidence -> Stage/SOP -> Skill Guard -> Artifact -> Review -> User Decision -> Revision/Re-review -> Project Memory -> Export`; it pauses with a durable `thread_id` rather than letting the coach skip ahead in text. A passing Gate still requires verified Skill evidence, complete required-role review evidence and a user confirmation; a graph checkpoint is not a pass by itself.
